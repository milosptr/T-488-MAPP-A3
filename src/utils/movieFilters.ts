import type { FiltersState } from '@/src/store/slices/filtersSlice';
import type { Movie, Showtime } from '@/src/types';
...
export const groupMoviesByCinema = (movies: Movie[]): CinemaGroup[] => {
    ...
    const seenMoviesByCinema = new Map<number, Set<string>>();
    ...
    movies.forEach(movie => {
        movie.showtimes.forEach((showtime: Showtime) => {
            ...
            if (!grouped.has(cinemaId)) {
                ...
                seenMoviesByCinema.set(cinemaId, new Set());
            }
            const seen = seenMoviesByCinema.get(cinemaId)!;
            if (!seen.has(movie._id)) {
                seen.add(movie._id);
                grouped.get(cinemaId)!.movies.push(movie);
            }
        });
    });
